...
/// Detailed commit information with author and timestamp
#[derive(Debug, Clone, Serialize, Deserialize, TS, schemars::JsonSchema)]
...
/// Response containing all commits for an attempt
#[derive(Debug, Serialize, Deserialize, TS, schemars::JsonSchema)]
pub struct AttemptCommitsResponse {
...
/// Get all commits for a task attempt with full metadata and diff statistics
pub async fn get_attempt_commits(
    Extension(task_attempt): Extension<TaskAttempt>,
    State(deployment): State<DeploymentImpl>,
) -> Result<ResponseJson<ApiResponse<AttemptCommitsResponse>>, ApiError> {
    let pool = &deployment.db().pool;
    ...
    let mut commits_details = Vec::new();
    let mut seen_commits = std::collections::HashSet::new();

    if let Some(ref wt_path) = worktree_path {
        let wt = wt_path.as_path();
            ...
            if let Some(commit_sha) = &process.after_head_commit {
                if !seen_commits.contains(commit_sha) {
                    ...
                    let message = deployment
                        .git()
                        .get_commit_subject(wt, commit_sha)
                        .unwrap_or_else(|_| commit_sha[..7].to_string());

                    // Get author information
                    let (author_name, author_email) = deployment
                        .git()
                        .get_commit_author(wt, commit_sha)
                        .unwrap_or((None, None));

                    // Get timestamp
                    let timestamp = deployment
                        .git()
                        .get_commit_timestamp(wt, commit_sha)
                        .ok()
                        .map(|dt| dt.to_rfc3339());

                    // Get diff statistics
                    let (files_changed, additions, deletions) = deployment
                        .git()
                        .get_commit_stats(wt, commit_sha)
                        .unwrap_or((0, 0, 0));

                    commits_details.push(CommitDetails {
                        sha: commit_sha.clone(),
                        message,
                        author_name,
                        author_email,
        ...
        // Worktree not available - return basic info from SHA only
        for process in &execution_processes {
            if let Some(commit_sha) = &process.after_head_commit {
                if !seen_commits.contains(commit_sha) {
                    seen_commits.insert(commit_sha.clone());
                    ...
                    commits_details.push(CommitDetails {
                        sha: commit_sha.clone(),
                        message: commit_sha[..7].to_string(),
                        author_name: None,
                        author_email: None,
                        timestamp: None,
                        files_changed: None,
    ...
    let total_count = commits_details.len();

    let response = AttemptCommitsResponse {
        attempt_id: task_attempt.id.to_string(),
